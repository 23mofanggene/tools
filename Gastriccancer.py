...
sql = "select data  from t_riskrate where rsid in (select rsid from t_all_23mofang_rsids where zhname='胃癌（胃贲门腺癌）') and sex='%d'" %(1)
try:
	# 执行SQL语句
	cursor.execute(sql)
						...
						for ff in rates[5]:
							for gg in rates[6]:
								for hh in rates[7]:
									for ii in rates[8]:
										for jj in rates[9]:			
											sumx=aa*bb*cc*dd*ee*ff*gg*hh*ii*jj
											ratex.append(sumx)
	print(ormx,ratex)
except:
